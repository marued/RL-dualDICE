...
class SarsaOffPolicy:
    def __init__(self, q_estimator, imp_sampler, discount_factor=1.0):
        self.q_estimator = q_estimator
        self.discount_factor = discount_factor
        self.imp_sampler = imp_sampler

    def update(self, env, target_policy, state, action):
        # Calculate next target
        next_state, reward, done, _ = env.step(action)
        next_action = target_policy(self.q_estimator, next_state)
        next_Q = self.q_estimator.value(next_state, next_action)
        if not done:
            target = reward + self.discount_factor * next_Q
        else: # TODO test difference between this if and no if...
        ...
        # Update QValue estimator
        self.imp_sampler.update()
        imp_value = self.imp_sampler.value(state, action)
        self.q_estimator.update(state, action, target, importance_sampling=imp_value)
        self.imp_sampler.post_update()
